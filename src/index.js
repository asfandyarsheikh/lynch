...
function executeJavaScriptWithDeps(scriptPath, args) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const tempProjectDir = join(tmpdir(), `lynch-project-${Date.now()}`);
      fs.mkdirSync(tempProjectDir);
      
      // Create package.json with dependencies
      const packageJson = {
        name: "lynch-temp-project",
        version: "1.0.0",
...
// Parse command line arguments (added by lynch)
...
// Execute the script function if it exists (added by lynch)
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: npx lynch <url|script-name> [args...]');
    console.error('');
    console.error('Examples:');
    console.error('  npx lynch https://example.com/script.sh arg1 arg2');
    console.error('  npx lynch my-script arg1 arg2  # downloads from n-p-x/e repo');
    console.error('  npx lynch repo/my-script arg1 arg2  # downloads from n-p-x/repo');
    console.error('');
  ...
  const scriptPath = join(tempDir, `lynch-${Date.now()}${fileExtension}`);
